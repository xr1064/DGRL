function W = constructWK2(fea,options,kerneloptions)
    ...
    switch lower(options.WeightMode)
        ...
        case {lower('HeatKernel')}
            if options.k > 0
                ...
                for i=1:nLabel
                    classIdx = find(options.gnd==Label(i));
                    K1 = constructKernel(fea(classIdx,:),[],kerneloptions);
                    K = constructKernel(fea,[],kerneloptions);
                    K1_K = constructKernel(fea(classIdx,:),fea,kerneloptions);
                    K_K1 = constructKernel(fea,fea(classIdx,:),kerneloptions);
                    K1_K1_Cent = K1-(1/nSmp)*K1_K*ones(nSmp,1)*ones(1,length(classIdx)) ...
                        -(1/nSmp)*ones(length(classIdx),1)*ones(1,nSmp)*K_K1 ...
                        +(1/nSmp)*(1/nSmp)*ones(length(classIdx),1)*ones(1,nSmp)*K*ones(nSmp,1)*ones(1,length(classIdx));
                    ...
                    D = bsxfun(@plus,aa,aa') - 2*K1_K1_Cent;
                    D(D<0) = 0;
                    D = max(D,D');
                    clear K1 K K1_K K_K1 K1_K1_Cent aa;
                    %===================================
                    ...
                    dump = (length(classIdx)/nSmp)^2*dump.*(1+dump);
