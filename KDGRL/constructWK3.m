function W = constructWK3(fea,options,kerneloptions)
...
if strcmpi(options.NeighborMode,'Supervised')
    ...
    switch lower(options.WeightMode)
        ...
        case {lower('HeatKernel')}
            if options.k > 0
                G = zeros(nSmp*(options.k),3);
                idNow = 0;
                for i=1:nLabel
                    classIdx = find(options.gnd==Label(i));
                    classIdy = find(options.gnd~=Label(i));
                    %D = EuDist2(fea(classIdx,:),fea(classIdy,:),0);
                    D = HibertDist2(fea(classIdx,:),fea(classIdy,:),kerneloptions,0);
                    [dump idx] = sort(D,2); % sort each row
                    clear D;
                    idx = idx(:,1:options.k);
                    dump = dump(:,1:options.k);
                    dump = exp(-dump/(2*options.t^2));
                    dump = (length(classIdx)/nSmp)*(length(classIdy)/nSmp)*dump.*(1-dump);
                    
                    nSmpClass = length(classIdx)*(options.k);
                    G(idNow+1:nSmpClass+idNow,1) = repmat(classIdx,[options.k,1]); %行坐标
                    G(idNow+1:nSmpClass+idNow,2) = classIdy(idx(:));   %列坐标
